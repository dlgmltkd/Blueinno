...
//	if ((ulPin > 0) && (ulPin < PINS_COUNT))	// Only pins 1-6 is avaliable for using as ADC inputs
	if (ulPin < PINS_COUNT)	// Only pins 0-6 is avaliable for using as ADC inputs
	{
...
void analogWrite(uint32_t ulPin, uint32_t ulValue) {
  ...
  if (ulValue == 255)
  {
	...
	else
	{
      ...
      attachIRQ(TIMER1_IRQn, TIMER1_Interrupt);
		...
		else
		{
          ...
          attachIRQ(TIMER2_IRQn, TIMER2_Interrupt);
